...
func (h *stateChangeHandler) HandleStateChange(ctx context.Context, from peanats.RaftState, to peanats.RaftState) error {
	h.log.Log(ctx, slog.LevelDebug-1, "state change", "from", from, "to", to)
	return nil
}

func worker(idx int) (peanats.RaftState, error) {
	...
	r, err := peanats.NewRaft(nc,
		peanats.RaftSize(3),
		peanats.RaftContext(ctx),
		peanats.WithLog(fmt.Sprintf(path.Join(os.TempDir(), "raft.%d.log"), idx)),
		peanats.RaftStateChangeHandler(&stateChangeHandler{log: log}),
	)
	if err != nil {
		return peanats.RaftStateFollower, err
	}

	var state peanats.RaftState

	state = r.State()
	log.Debug("reached", slog.String("state", state.String()))

	err = r.Wait(ctx, peanats.RaftStateCandidate)
	if err != nil {
		return state, err
	}
	...
	err = r.Wait(ctx, peanats.RaftStateLeader)
	if err != nil {
		return state, err
	}
