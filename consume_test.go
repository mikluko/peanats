...
func BenchmarkConsume(b *testing.B) {
	b.Run("single/json", func(b *testing.B) {
		benchmarkConsume(b, peanats.JsonCodec{})
	})
	b.Run("single/msgpack", func(b *testing.B) {
		benchmarkConsume(b, peanats.MsgpackCodec{})
	})
	// worker pool is expected to be slower vs single-threaded on no-op benchmark handler
	b.Run("pool/json", func(b *testing.B) {
		benchmarkConsume(b, peanats.JsonCodec{}, peanats.ConsumeSubmitter(pond.Submitter(250)))
	})
	b.Run("pool/msgpack", func(b *testing.B) {
		benchmarkConsume(b, peanats.MsgpackCodec{}, peanats.ConsumeSubmitter(pond.Submitter(250)))
	})
}
...
func benchmarkConsume(b *testing.B, codec peanats.Codec, opts ...peanats.ConsumeOption) {
	...
	h := peanats.ArgumentHandlerFunc[benchmarkConsumeArgument](
		func(ctx context.Context, a peanats.Argument[benchmarkConsumeArgument]) error {
			defer wg.Done()
	...
	err = peanats.Consume(b.Context(), c, peanats.ArgumentMessageHandler(h), opts...)
	if err != nil {
		b.Fatal(err)
	}
