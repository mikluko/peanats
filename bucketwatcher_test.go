...
func TestWatcher(t *testing.T) {
	...
	nw := newKeyWatcherMock(t)
	nw.EXPECT().Updates().Return(ch)

	w := peanats.NewBucketWatcher[testModel](nw, peanats.BucketWatcherPrefix("parson"))
	...
	e, err = w.Next()
	require.ErrorIs(t, err, peanats.ErrInitialValuesOver)
	assert.Nil(t, e)
...
func TestWatch(t *testing.T) {
	t.Run("happy path", func(t *testing.T) {
		m := newBucketEntryMock[testModel](t)
		m.EXPECT().Key().Return("parson.had.a.dog")
		m.EXPECT().Value().Return(&testModel{Name: "balooney"})

		w := newBucketWatcherMock[testModel](t)

		w.EXPECT().Next().Return(m, nil).Once()
		w.EXPECT().Next().Return(nil, peanats.ErrInitialValuesOver).Once()
		w.EXPECT().Next().Return(m, nil).Once()
		...
		h := peanats.BucketEntryHandlerFunc[testModel](func(ctx context.Context, b peanats.BucketEntry[testModel]) error {
		...
		err := peanats.Watch(t.Context(), w, h)
		require.NoError(t, err)
