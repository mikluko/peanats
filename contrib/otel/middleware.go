...
func Middleware(opts ...MiddlewareOption) peanats.MessageMiddleware {
	p := middlewareParams{
	...
	return func(next peanats.MessageHandler) peanats.MessageHandler {
		return peanats.MessageHandlerFunc(func(ctx context.Context, msg peanats.Message) error {
			...
			if msgx, ok := msg.(peanats.Metadatable); ok {
				attrs = append(attrs, MessageMetadataAttributes(msgx.Metadata())...)
			}
