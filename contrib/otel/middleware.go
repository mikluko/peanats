...
func Middleware(opts ...MiddlewareOption) xmsg.MsgMiddleware {
	p := middlewareParams{
	...
	return func(next xmsg.MsgHandler) xmsg.MsgHandler {
		return xmsg.MsgHandlerFunc(func(ctx context.Context, msg xmsg.Msg) error {
			...
			if msgx, ok := msg.(xmsg.Metadatable); ok {
